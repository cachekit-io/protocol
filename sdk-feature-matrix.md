...
<!-- -->
> ⁷ **Python has no generic backend-operation retry** (LAB-522). What exists is Redis-client-level reconnect/timeout retry and lock-acquisition retry. Setting `max_retries` does nothing: the field on `CachekitConfig` (`config/settings.py:200`) has no **operational** consumer — its only read is a validator branch whose body is `pass` (`settings.py:412`) — and the CachekitIO backend's own `max_retries` (`backends/cachekitio/config.py:121`) has no reader at all (through `cachekit` 0.19.0, wheel inspected 2026-09-23; the open [cachekit-py#324](https://github.com/cachekit-io/cachekit-py/pull/324) deletes the `CachekitConfig` field and its `pass` branch but leaves the CachekitIO field, and this sentence changes when a PyPI release carries it). Rust and TypeScript both wrap backend ops in a real retry layer.
>
