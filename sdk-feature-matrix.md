...
*Last updated: 2026-07-22 — LAB-431 Workers feasibility spike: ts Cloudflare Workers footnote records GO verdict (WebCrypto path), implementation tracked as LAB-595*
...
> ¹ The TS SDK cannot run on the Workers runtime today: crypto is a native NAPI module and the Redis backend is ioredis (TCP). Recorded as ❌ (a plausible target, unsupported) rather than N/A. Feasibility spike LAB-431 (2026-07-22) returned **GO**: WebCrypto covers AES-256-GCM + HKDF-SHA256 natively (random 12-byte nonces are permitted as a fallback for SDKs without counter nonces, see [encryption.md → Nonce Generation](spec/encryption.md#nonce-generation)), the CachekitIO backend is already pure `fetch`, and a planned `workerd`-conditional entrypoint can keep ioredis/NAPI out of the edge bundle. Implementation tracked as LAB-595 (compression stays off on Workers in phase 1 — the ByteStorage envelope is NAPI-bound).
>
