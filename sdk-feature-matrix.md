...
> [!NOTE]
> Unprefixed keys — every TS or Rust SDK key, every interop-mode key, any bare hash — all resolve to the single server-side `default` namespace and share its `keyClass: 'open'` write space, open to both `ck_sdk_` and `ck_api_` callers alike (`cache-key-validator.ts:56-59,118-120`). They are still subject to whatever `allowed_namespaces` grant applies to `default`; "open" describes the writer-class boundary, not an absence of authorization. What they cannot do is scope traffic to anything narrower than `default` — that segmentation exists only for `ns:`/`nsapi:`-prefixed keys. This is current behavior, not a recommendation — whether TS/Rust should adopt `ns:`-style prefixing to gain per-namespace segmentation is LAB-640's decision, not this document's.
