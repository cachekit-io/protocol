...
*Last updated: 2026-07-23 — LAB-427: TypeScript Redis backend implements `TTLBackend` + `LockableBackend`; ts Redis TTL/locking cells flipped ✅*
...
| :--- | :--- | :--- | :--- |
| TTL inspect / refresh | `TTLInspectableBackend` — Redis ✅, SaaS ✅, Memcached/File ❌ | `TtlInspectable` — Redis ✅, SaaS ✅, Workers ❌ | `TTLBackend` — SaaS ✅ (`TTLCachekitIO`), Redis ✅ (`RedisBackend.getTTL`/`refreshTTL`, LAB-427) |
| Distributed locking | `LockableBackend` — Redis ✅ (`redis.lock.Lock`), SaaS ✅ | `LockableBackend` — SaaS ✅, Redis ❌, Workers ❌ | `LockableBackend` — SaaS ✅ (`LockableCachekitIO`), Redis ✅ (`SET NX PX` + Lua compare-and-delete, LAB-427) |
...
>
> **Coverage, not shape, is the parity gap:** all three SDKs use the same required-base + optional-capability pattern, but Redis optional-capability coverage varies by SDK: Python and TypeScript cover both locking and TTL inspection on Redis (ts closed its gap in LAB-427: `packages/cachekit/src/backends/redis.ts` implements `TTLBackend` + `LockableBackend`, bare-key `:lock` derivation identical to py), while Rust's Redis backend implements TTL inspection only (no locking — LAB-426). Rust's Workers backend also lacks both despite speaking the same SaaS API (gap tickets under LAB-102).
...
| Distributed locking | ✅ Redis + SaaS backends | ✅ SaaS backend only (`LockableBackend`; Redis/Workers lack impls) | ✅ Redis + SaaS backends | ❌ |
| L1/L2 dual-layer cache | ✅ | ✅ moka (native) / `l1` feature | ✅ | ❌ |
| Cache stampede prevention | ✅ | ❌ | ✅ Version tokens + background L1 refresh | ❌ |
| TTL management | ✅ Redis + SaaS | ✅ Redis + SaaS (`TtlInspectable`; Workers ❌) | ✅ Redis + SaaS (`TTLBackend`) | ❌ |
