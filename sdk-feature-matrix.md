...
"Wired" means the live cache path feeds it with zero user plumbing. Audited against code 2026-08-04 (LAB-1400). Rust column re-verified against [cachekit-rs#81](https://github.com/cachekit-io/cachekit-rs/pull/81) on 2026-09-22 (LAB-521) — 🚧 until that lands on crates.io (0.7.0 does not have it).

| Feature | Python | Rust | TypeScript | PHP |
| :--- | :--- | :--- | :--- | :--- |
| Metrics | ✅ Live `prometheus_client` counters / gauges / histograms on the default registry; no HTTP exposition helper | 🚧 Live counters ([cachekit-rs#81](https://github.com/cachekit-io/cachekit-rs/pull/81), unreleased) — `CacheKit::stats()` returns `L1Stats` (`l1_hits` / `l2_hits` / `misses`), incremented by the client read path (`client.rs` `get_bytes` / `get_bytes_swr`), plus `l1_entry_count()` and `circuit_state()`; no Prometheus exposition — Rust services bring their own registry (LAB-521) | ✅ Node — live Prometheus metrics via the optional `prom-client` peer dep, custom registry supported, warns once and no-ops if absent (LAB-517) · ❌ Workers — `prom-client` is CI-excluded from the edge bundle¹ | ❌ |
| Structured logging | ✅ JSON ring-buffer logger with sensitive-data masking (`logging.py`) | 🚧 ([cachekit-rs#81](https://github.com/cachekit-io/cachekit-rs/pull/81), unreleased) `tracing` events behind the opt-in `tracing` cargo feature: one `debug` event per get/set/delete on target `cachekit` (`op`, `outcome`, `key_hash` = Blake2b-128 of the storage key — never the key), breaker transitions on `cachekit::reliability` (`metrics.rs`, `reliability.rs`). No logger of its own — events go to the host's subscriber, the Rust idiom (LAB-521) | ⚠️ Pluggable error-logger hook (`setLogger`, default `console.error`) — a logging seam, not a structured logger (LAB-517) | ❌ |
| Distributed tracing (OTel) | ❌ The span-shaped API accepts spans and discards them (`NoOpSpan`, `decorators/orchestrator.py:260-267`) — instrumentation built against it silently produces nothing | ❌ Events, not spans — bridgeable via `tracing-opentelemetry` in the host | ❌ | ❌ |
| SaaS telemetry headers (`X-CacheKit-L1-*`) | ✅ Auto | 🚧 Auto ([cachekit-rs#81](https://github.com/cachekit-io/cachekit-rs/pull/81), unreleased) — `CacheKitBuilder::build()` attaches the client's live counters through `Backend::attach_metrics` (`backend/mod.rs`); an explicit `.metrics_provider()` on the backend builder still wins. `L1-Status` now sends the server-accepted `miss` (was `enabled`, which the SaaS rejects with 400 for SDK keys) (LAB-521) | ✅ Auto-wired from the live L1/L2 hit/miss counters; an explicit user `metricsProvider` still wins | ❌ |
