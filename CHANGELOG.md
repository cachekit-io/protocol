...
- StorageEnvelope `compressed_data` canonical encoding flipped from MessagePack
  ...
  under rmp-serde (toolchain-verified, including `bin` wire through the shipped
  `ByteStorage::retrieve()`); no version field or discriminator introduced.
  `checksum` deliberately stays array-of-ints; `format` untouched (feeds AAD
  v0x03, which contains no envelope bytes). Tiny envelopes may grow ≤ +4 B
  (`empty` 25 → 26 B); incompressible payloads shrink ~35% (1.508× → 1.0039×)
  with a 6–11× codec-throughput win. Rationale, rejected options, and rollout
...
- 6 `bin`-encoded twins appended to `test-vectors/wire-format.json` (append-only;
  the 6 legacy array-of-ints vectors are retained forever as legacy-read proof;
  fixture version 1.0.0 → 1.1.0). Twins generated by the stdlib-only
  `tools/wire-format-reference.py` and byte-verified against real
  `rmp-serde 1.3.1 + serde_bytes 0.11.19` output; `verify` now runs in CI
  (stdlib pass + `msgpack` third-encoder conformance) — the wire-format
  fixture's first protocol-side CI verification.
