...
- **`spec/interop-v2.md`'s rationale corrected**: 32-bit wrapping was said to
  ...
  the failure mode is **spurious rejection**, never a bomb bypass — a hard
  error rather than a cache miss, and permanent for an affected entry. The
  rejection is not uniform (a density of `2²⁹/2³² = 1/8` over the legal range),
  but where it bites the collapse is near-total: 704 B at the threshold, 8 B at
  its floor, 0 at the 512 MiB cap. The 99.2 % figure four bullets above measures
  *reject-on-overflow*, a different mechanism, and is unaffected. Both documents
  now say so, marked non-normative, and interop v2 records the corrected claim
  in place.
