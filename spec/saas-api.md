...
| `401` | Unauthorized | Invalid or missing API key. An authoritative verdict from the key store — never emitted for a backend fault while checking the key (that is a `503`) |
