...
Without `X-CacheKit-TTL`, the entry has **no expiry**: no `fresh_until`, no `evict_at`. It is served `200 OK` `fresh` until deleted or overwritten, never enters the stale window, and carries no `X-CacheKit-Fresh-For` ([Remaining Freshness](#remaining-freshness)). `X-CacheKit-Stale-TTL` without `X-CacheKit-TTL` is rejected ([validation](#validation)) — an entry that never expires has nothing to be stale relative to.
...
- **`GET /v1/cache/{key}/ttl`:** the returned `ttl` is the remaining seconds until **eviction** (`evict_at`), or `null` for a no-expiry entry.
