...
- A **signal-capable server** is one that implements this section. It is independent of [stale-while-revalidate](#stale-while-revalidate) support: every entry has a `fresh_until`, so a server may emit `X-CacheKit-Fresh-For` without offering a stale window, and a pre-signal SWR server emits `X-CacheKit-Freshness` without it.
...
- Omitted only by pre-signal servers. A re-serving tier fronting a pre-signal store is itself pre-signal and passes the absence through.
