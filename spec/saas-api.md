...
| :--- | :---: | :--- |
| `X-CacheKit-TTL` | No | Time-to-live in seconds. Positive integer, minimum 1, maximum 2,592,000 (30 days). Omit — together with legacy `X-TTL` — to store the entry with **no expiry** (see the **No-expiry contract** under TTL Validation Rules); servers MUST NOT substitute a hidden default. |
