...
*Protocol Version 1.0 · Verified against `cachekit-py` v0.18.0 (`src/cachekit/key_generator.py`); serializer-code derivation verified against [cachekit-io/cachekit-py#311](https://github.com/cachekit-io/cachekit-py/pull/311) @ `3fd7c35` (open); vectors generated by v0.12.0 (see [Test Vectors](#test-vectors))*
...
>
> **On every read, before decoding the payload, the reader MUST compare the serializer name
> its storage container records (Python: the `s` field of the
...
> cannot exist (`cachekit-ts` and `cachekit-rs` record none and offer one).
>
...
> `default` and `pythonic` for `auto`, canonicalizing them before the lookup. A serializer
> passed as an *instance* rather than a name is recorded in the frame header under its bare
...
// CONFORMANCE: test-vectors/cache-keys.json records serializer_type "std", which is
// cachekit-py's alias for the canonical "default". An SDK that does not accept "std" MUST
// read those vectors as serializer_type = "default" (-> code "s"); it MUST NOT add the alias
// to its own map to pass them. Read as an unaliased identity, "std" derives
// "x" + blake2b("std") and fails all 10 vectors.
